...
class Ui_Test_T_Deux_Ech_Ind(QMainWindow):
    def __init__(self, tabView: (QTabWidget), webView: (QWebEngineView)):
        ...
        self.cBNomVar_1.addItems(varName for varName, varType in zip(dm.dictGlobalData[dm.key_main_column_dtype_dict].keys(), dm.dictGlobalData[dm.key_main_column_dtype_dict].values()) if varType == dm.var_binaire)
    ...
    def onClickValider(self):
        ...
        if variableName_1 != "" and variableName_2 != "":
            ...
            nObs1 = len(variable_1)
            nObs2 = len(variable_2)
            
            interTitle = ""
            testLaterale = "two-sided"
            
            moyenne_1 = np.mean(variable_1)
            devStd_1 = np.std(variable_1, ddof=1)
            
            moyenne_2 = np.mean(variable_2)
            devStd_2 = np.std(variable_2, ddof=1)
            ...
            t = stats.ttest_ind(a= variable_1, b= variable_2, alternative=testLaterale)
            ...
            partieList = partieList + rg.formatTableHPartie(self.refCount, interTitle, [[variableName_1, "N_Obs", "Moyenne","Ec_Type", "Err_Type","minimum", "maximum"], [groupes_uniques[0],  int(len(variable_1)), rg.formatFloat(np.mean(variable_1)), rg.formatFloat(devStd_1), rg.formatFloat(stats.sem(variable_1, ddof= 1)), rg.formatFloat(variable_1.min()), rg.formatFloat(variable_1.max())], [groupes_uniques[1], int(len(variable_2)), rg.formatFloat(np.mean(variable_2)), rg.formatFloat(devStd_2), rg.formatFloat(stats.sem(variable_2, ddof= 1)), rg.formatFloat(variable_2.min()), rg.formatFloat(variable_2.max())]])
            self.refCount= self.refCount +1
                
            #Tableau du test de T Student
            interTitle  = "Tableau du test T Stuedent à deux échantillons indépendantes de {} suivant les modalitées de  {}".format(variableName_2, variableName_1)
            pointList = pointList + rg.formatPoint(self.refCount, interTitle, rg.tableaIconStr)
            ...
            testTitle = "Test T Stuedent à deux échantillons indépendantes de {} suivant les modalitées de  {}".format(variableName_2, variableName_1)
